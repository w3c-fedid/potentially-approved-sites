...
We use `potentially_approved_site_hashes` rather than `potentially_approved_site` because it leads to fewer bytes sent back to the browser. Because registered domain names is easily enumerable (making the hashes easily reversible), we don't use hashes to make them opaque.
